...
class AgentOrchestrator:
    ...
    def _build_context(self, task: str, context: Optional[Dict[str, Any]] = None) -> AgentContext:
        ...
        if context:
            ...
            allowed_stock_codes = context.get("allowed_stock_codes")
            if isinstance(allowed_stock_codes, list):
                ctx.allowed_stock_codes = [str(code).strip() for code in allowed_stock_codes if str(code).strip()]
            allowed_stocks = context.get("allowed_stocks")
            if isinstance(allowed_stocks, list):
                ctx.allowed_stocks = [
                    {
                        "stock_code": str(item.get("stock_code") or item.get("code") or "").strip(),
                        "stock_name": str(item.get("stock_name") or item.get("name") or "").strip(),
                    }
                    for item in allowed_stocks
                    if isinstance(item, dict) and str(item.get("stock_code") or item.get("code") or "").strip()
                ]
