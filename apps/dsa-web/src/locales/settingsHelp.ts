...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.ai_model.LLM_USAGE_HMAC_SECRET': {
    ...
    usage: '通常留空即可，系统会在数据目录生成本地密钥文件；只有需要跨部署比较 HMAC 时才手动配置同一个随机密钥。',
    valueNotes: [
    ...
    notes: ['不要使用登录 session secret，也不要在 issue、日志或截图里暴露该密钥。'],
  },
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.ai_model.LLM_USAGE_HMAC_SECRET': {
    ...
    usage: 'Usually leave this empty so the backend creates a local secret file. Set it only when deployments intentionally need comparable HMACs.',
    valueNotes: [
    ...
    notes: ['Do not reuse the login session secret or expose this value in issues, logs, or screenshots.'],
  },
  'settings.ai_model.LLM_USAGE_HMAC_KEY_VERSION': {
