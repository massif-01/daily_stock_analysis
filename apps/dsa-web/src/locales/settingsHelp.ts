...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.ai_model.GENERATION_BACKEND': {
    ...
    usage: '通常保持“默认模型配置”。只有在你信任本机 Codex CLI runtime 并已完成登录时，才显式选择 Codex CLI（experimental/limited）。',
    valueNotes: [
      'Codex CLI 是本地进程执行能力，不等于离线模型；CLI 背后的服务可能处理股票代码、新闻、持仓上下文、分析 prompt 和报告草稿。',
      'Docker、云服务器、CI 不天然拥有你本机的 CLI 登录态；DSA 不读取 Codex credential 文件，但子进程可能使用 CLI 自身登录态。',
    ],
    impact: ['影响普通分析、大盘复盘和文本生成入口，不改变问股助手的工具执行规则。'],
    notes: [
      '想恢复默认行为，选择“默认模型配置”并保存配置。',
      'Codex CLI 当前为 experimental/limited；如果非交互 stdout contract 不稳定，请设回默认模型配置。',
      '高级说明：默认模型配置内部由 LiteLLM 兼容层执行；普通使用无需了解或修改该内部值。',
    ],
    examples: [],
  },
  'settings.ai_model.GENERATION_FALLBACK_BACKEND': {
    title: '备用生成方式',
    showFieldKey: false,
    summary: '控制本地 CLI backend 失败时是否回退到默认 LiteLLM 模型配置。',
    usage: '留空表示禁用 backend-level fallback；选择“默认模型配置”表示 local CLI 失败后可回退 LiteLLM。',
    valueNotes: [
      '如果只是想设置主模型失败后的备用模型，请使用“备选模型”，不是这个字段。',
      '主生成方式本身就是默认模型配置时，不会执行 self fallback。',
    ],
    impact: ['不改变现有模型备用顺序，也不会影响渠道编辑器里的模型配置。'],
    notes: [
      '需要 fail-fast 时留空；需要 local CLI 失败后继续尝试云端模型时选择默认模型配置。',
    ],
    examples: [],
  },
  'settings.ai_model.GENERATION_BACKEND_TIMEOUT_SECONDS': {
    title: '生成 Backend 超时秒数',
    summary: '限制单次生成 backend 调用的最长执行时间。',
    usage: '主要用于本地 CLI backend，默认 300 秒。LiteLLM Router、渠道和 MAX_WORKERS 行为不受此字段改变。',
    valueNotes: ['超时后会清理 CLI 进程组，并返回结构化 timeout 错误。'],
  },
  'settings.ai_model.GENERATION_BACKEND_MAX_OUTPUT_BYTES': {
    title: '生成 Backend 最大输出字节数',
    summary: '限制本地 CLI 调用可捕获的 stdout/stderr 规模。',
    usage: '默认 1048576 字节。超过限制时返回 output_too_large，不继续解析输出。',
    valueNotes: ['日志和 diagnostics 只展示脱敏截断预览，不展示完整 prompt、env、stderr 或 executable path。'],
  },
  'settings.ai_model.GENERATION_BACKEND_MAX_CONCURRENCY': {
    title: '生成 Backend 最大并发',
    summary: '限制 generation backend 的全局并发上限。',
    usage: '本地 CLI 的有效并发为该值与 LOCAL_CLI_BACKEND_MAX_CONCURRENCY 的较小值，默认 1。',
    valueNotes: ['GENERATION_BACKEND=litellm 时不改变现有 MAX_WORKERS 或 LiteLLM Router 行为。'],
  },
  'settings.ai_model.LOCAL_CLI_BACKEND_MAX_CONCURRENCY': {
    title: '本地 CLI Backend 最大并发',
    summary: '限制本地 CLI backend 的并发执行数量。',
    usage: '默认 1，避免同时启动多个本地 coding-agent 进程。',
    valueNotes: ['有效并发为 min(LOCAL_CLI_BACKEND_MAX_CONCURRENCY, GENERATION_BACKEND_MAX_CONCURRENCY)。'],
  },
  ...
  'settings.agent.AGENT_GENERATION_BACKEND': {
    ...
    valueNotes: [
      ...
      'codex_cli 在 Phase 2 不支持 Agent 工具调用；显式选择会提示该能力不可用，或按配置回退 LiteLLM。',
    ],
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.ai_model.GENERATION_BACKEND': {
    ...
    usage: 'Usually keep Default model settings. Choose Codex CLI (experimental/limited) only when you trust the local CLI runtime and have completed CLI login.',
    valueNotes: [
      'Codex CLI is a local process backend, not an offline model. The service behind the CLI may process stock symbols, news, position context, analysis prompts, and report drafts.',
      'Docker, cloud servers, and CI do not automatically have your local CLI login state. DSA does not read Codex credential files, but the subprocess may use the CLI login state.',
    ],
    ...
    notes: [
      ...
      'Codex CLI is experimental/limited. If the non-interactive stdout contract is unstable, switch back to Default model settings.',
      'Advanced note: Default model settings are executed through the LiteLLM compatibility layer; regular users do not need to understand or change that internal value.',
    ],
    examples: [],
  },
  'settings.ai_model.GENERATION_FALLBACK_BACKEND': {
    ...
    showFieldKey: false,
    summary: 'Controls whether local CLI backend failures can fall back to the default LiteLLM model settings.',
    usage: 'An empty value disables backend-level fallback. Default model settings lets local CLI failures fall back to LiteLLM.',
    valueNotes: [
      'Use fallback models for LiteLLM model-to-model fallback; this field is backend-level fallback.',
      'When the primary generation backend is already LiteLLM, self fallback is ignored.',
    ],
    impact: ['Affects local CLI failure handling for stock analysis, market review, and free-form text generation.'],
    notes: [
      'Leave it empty for fail-fast local CLI behavior, or use Default model settings to recover through LiteLLM.',
    ],
    examples: [],
  },
  'settings.ai_model.GENERATION_BACKEND_TIMEOUT_SECONDS': {
    title: 'Generation Backend Timeout',
    summary: 'Limits the maximum duration of one generation backend call.',
    usage: 'Mainly applies to local CLI backends. The default is 300 seconds; LiteLLM Router and MAX_WORKERS behavior is unchanged.',
    valueNotes: ['Timeout cleans up the CLI process group and returns a structured timeout error.'],
  },
  'settings.ai_model.GENERATION_BACKEND_MAX_OUTPUT_BYTES': {
    title: 'Generation Backend Max Output Bytes',
    summary: 'Limits captured stdout/stderr size for local CLI backend calls.',
    usage: 'The default is 1048576 bytes. Oversized output returns output_too_large and is not parsed.',
    valueNotes: ['Diagnostics only expose redacted, truncated previews, never full prompts, env, stderr, or executable paths.'],
  },
  'settings.ai_model.GENERATION_BACKEND_MAX_CONCURRENCY': {
    title: 'Generation Backend Max Concurrency',
    summary: 'Sets the global generation backend concurrency cap.',
    usage: 'Local CLI effective concurrency is the lower of this value and LOCAL_CLI_BACKEND_MAX_CONCURRENCY. Default is 1.',
    valueNotes: ['When GENERATION_BACKEND=litellm, this does not change existing MAX_WORKERS or LiteLLM Router behavior.'],
  },
  'settings.ai_model.LOCAL_CLI_BACKEND_MAX_CONCURRENCY': {
    title: 'Local CLI Backend Max Concurrency',
    summary: 'Limits concurrent local CLI backend executions.',
    usage: 'Default is 1 to avoid starting multiple local coding-agent processes at once.',
    valueNotes: ['Effective concurrency is min(LOCAL_CLI_BACKEND_MAX_CONCURRENCY, GENERATION_BACKEND_MAX_CONCURRENCY).'],
  },
  'settings.ai_model.LITELLM_MODEL': {
    title: 'Primary Model',
  ...
  'settings.agent.AGENT_GENERATION_BACKEND': {
    ...
    valueNotes: [
      ...
      'codex_cli does not support Agent tool calling in Phase 2; explicit selection reports the capability as unavailable or falls back to LiteLLM.',
    ],
