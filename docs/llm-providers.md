...
Generation backend 配置是更外层的运行时选择契约。Phase 2 支持 `GENERATION_BACKEND=litellm|codex_cli`，但 `codex_cli` 是本地 CLI backend，不是 LiteLLM provider；不要配置成 `LITELLM_MODEL=codex_cli/...`。`codex_cli` preset 使用 `codex exec --output-last-message <temp-file> -` 读取最终响应，stdout/stderr 只作为诊断预览，并与最终响应一起计入 `GENERATION_BACKEND_MAX_OUTPUT_BYTES` 总上限；超限时返回结构化 `output_too_large`。`GENERATION_FALLBACK_BACKEND=` 空值会禁用 backend-level fallback，未配置时默认回退到 `litellm`。Agent 工具调用仍使用 LiteLLM；Web 设置页只暴露 `AGENT_GENERATION_BACKEND=auto|litellm`，手写 `codex_cli` 不会启用 text-only Agent mode，只会返回明确 unsupported tool-calling 诊断。
