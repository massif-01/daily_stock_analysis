...
`model_access_denied` 不是跨 provider 的官方错误码映射。本项目目前没有 SiliconFlow / OpenAI-compatible / LiteLLM 对 `Model disabled` 语义的官方错误码文档来源；该分类依据来自 Issue #1208 中真实脱敏样例：SiliconFlow / OpenAI Compatible 渠道测试经 LiteLLM 返回 `litellm.APIError: APIError: OpenAIException - Model disabled.`。实现仅在错误文本同时包含 `model` 和明确权限、禁用或不可用信号时进入该诊断；未覆盖或语义不同的 provider 文案会继续走既有兜底诊断。
