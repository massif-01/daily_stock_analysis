...
| Hermes 本地 HTTP | `hermes` | `openai` | `http://127.0.0.1:8642/v1` | `hermes-agent` |
...
| Hermes 本地 HTTP | [LiteLLM OpenAI-compatible](https://docs.litellm.ai/docs/providers/openai_compatible) | Phase 3 P0 只按 OpenAI-compatible channel 形状接入主生成路径；Base URL 必须是 `127.0.0.1`、`localhost` 或 `::1` loopback，非法 Hermes 配置会返回结构化错误且不会静默 fallback 到 legacy provider。tools probe 只代表 tool-call shape diagnostic，不代表 DSA tool result roundtrip；Hermes 不作为 AgentBackend 或 Agent tools-ready 模型暴露。 |
...
Hermes 默认 Base URL `http://127.0.0.1:8642/v1` 只允许 loopback client target。GitHub-hosted runner 上的 `127.0.0.1` 指向 runner 自身，不是你的个人电脑；若要在每日 workflow 中使用 Hermes，需要使用已启动 Hermes runtime 的 self-hosted runner，或在 workflow 内显式启动对应本地服务。
...
- Hermes：删除 `LLM_CHANNELS=hermes`，或恢复原 `LLM_CHANNELS` / `LITELLM_MODEL`；如需 Agent，继续使用独立的已验证 `AGENT_LITELLM_MODEL`。
