...
- [修复] #1390 收紧建议动作 legacy fallback：英文 `not to ...` 否定不再误判为买卖动作，Web 旧记录不再把中文金融上下文、`buy or sell`、多 guard 歧义文本或 `buyback` / `buy-back` / `buy back` / `selloff` / `sell-off` / `sell off` 等英文复合词渲染成 action badge，并在回测/历史趋势等入口按界面语言显示结构化 action 标签。
