...
- After opening ask-stock from a historical report, the Web/Desktop main path now keeps sending the current `stock_code` / `stock_name` as structured session context. One-shot report fields such as `previous_*` are reused only on the first follow-up turn and are not injected repeatedly.
- When request context already has an active stock, the Agent runner validates stock-scoped tool arguments before execution: financial/technical abbreviations such as `TTM`, `PE`, and `MACD` stay scoped to the active stock, while unrequested tool calls for another stock return `stock_scope_conflict` and are recorded in the tool call log. Bot free chat still keeps the previous behavior when no `context.stock_code` is provided; sticky Bot context remains a follow-up.
