...
持仓页会把 AI 建议作为非阻断增强异步加载：组合快照和风险模块先按原逻辑渲染，随后通过 `GET /api/v1/decision-signals?holding_only=true&status=active&page_size=100` 分页读取当前 active 持仓相关信号，按 `total` 取全，不做固定页数截断。若分页中途失败，页面保留已加载的部分信号并显示可见降级提示；无匹配信号时持仓行显示空占位。匹配逻辑复用 Web 端股票代码等价规则，覆盖 A 股 `600519/SH600519/600519.SH`、港股 `00700/HK00700/00700.HK` 和美股大小写 ticker。
