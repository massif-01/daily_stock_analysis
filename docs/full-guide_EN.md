...
The portfolio page loads AI signals as a non-blocking enhancement: portfolio snapshots and risk cards render first, then the page paginates through `GET /api/v1/decision-signals?holding_only=true&status=active&page_size=100` until `total` is covered, without a fixed page-count cutoff. If pagination fails mid-stream, the page keeps already loaded signals and shows a visible degradation warning; rows without a matching signal show an empty placeholder. Matching reuses the Web stock-code equivalence rules for CN variants such as `600519/SH600519/600519.SH`, HK variants such as `00700/HK00700/00700.HK`, and case-insensitive US tickers.
